...
export function AssessmentForm({ initialSubjects }: AssessmentFormProps) {
    ...
    React.useEffect(() => {
        if (initialSubjects.length > 0 && selectedSubjectIds.length === 0) {
            setSelectedSubjectIds(initialSubjects.map(s => s.id));
            const allChapterIds = initialSubjects.flatMap(s => s.chapters.map(c => c.id));
    ...
    const toggleAllSubjects = () => {
        if (selectedSubjectIds.length === initialSubjects.length) {
    ...
    const selectedSubjects = useMemo(() =>
        initialSubjects.filter((s) => selectedSubjectIds.includes(s.id)),
        [initialSubjects, selectedSubjectIds]);
    ...
    const activeSelectedChapterCount = useMemo(() => {
        return initialSubjects
            ...
            .length;
    }, [initialSubjects, selectedSubjectIds, selectedChapters]);
                            ...
                            >
                                <div
                                    className={cn(
                                        ...
                                        selectedSubjectIds.length === initialSubjects.length ? "bg-[#6366f1] border-[#6366f1]" : "border-gray-300 bg-white"
                                    )}
                                >
                                    {selectedSubjectIds.length === initialSubjects.length && <div className="w-2 h-2 bg-white rounded-full"></div>}
                                </div>
